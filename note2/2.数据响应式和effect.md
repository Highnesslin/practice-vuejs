...
**Vue3** 重写 **响应式原理**，解决了这些问题，速度提升**一倍**，内存占用减少**1/2**
...
由于**Vue3**依然兼容**Vue2**，所以原本的**options API**可以继续使用，经过**debug**调试后最终发现在`resolveData`方法中依然是调用了`reactive`；还有我在调试`ref`时发现最终也是调用了`reactive`，所以可以认为`reactive`是**Vue3 数据响应式**的**入口**。

但是在研究`reactive`之前，我们先看一下**响应式数据**的几种**枚举类型**
...
- 简化后的核心源码如下
  ...
  `getter`触发`track`实现**依赖收集**，`setter`触发`trigger`负责**触发依赖**。
