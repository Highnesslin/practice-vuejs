...
**初始化**的**方式**也发生了改变，通过**Vue**实例上的`createApp`方法创建**页面应用**，所以**初始化阶段**我们从这里入手
...
- 作用
  ...
  - 获取**渲染器**（`renderer`），通过**渲染器**的`createApp`方法创建**app 对象**
  - 扩展`mount`方法
...
- 核心源码
  ...
  这是基于`runtime-dom`的`createApp`，目的是通过`ensureRenderer`调用`runtime-dom`模块创建一个基于**web 平台**的**渲染器**

  `rendererOptions`是**web 平台**特有的操作**dom**和**属性**的方法。
...
- 核心源码
  ...
  这里再调用一次`baseCreateRenderer`是为了创建**客户端渲染器**，可以看到还有另外一个方法`createHydrationRenderer`也是调用了`baseCreateRenderer`，这是**服务端渲染器**
...
- 核心源码
  ...
  **渲染器**包括`render`、`hydrate`、`createApp`三个方法，<br>
  ...
  通过参数`options`解构出**基于平台**的**操作** **dom** 与 **属性** 的方法，用来创建真正的**渲染**和**更新**相关的方法，其中需要关注的是`patch`方法，因为他不止负责**渲染**、**更新**，将来的**初始化组件**也通过这个入口进入 ⭐。

  其中`render`方法类似于**vue2**的`vm._update`，负责**初始化**和**更新**

  由于`baseCreateRenderer`是一个长达 1800 多行的方法，**初始化时**只关注最终返回的**渲染器**即可，

  `createApp`由工厂函数`createAppAPI`创建
...
- 作用
  ...
  - 通过参数`render`和`hydrate`创建**平台**的`createApp`方法，用于创建真正的**app 实例**，也就是**Vue 实例**
...
- 核心源码
  ...
  这一步结束后**渲染器**的`createApp`方法也有了，`ensureRenderer`将整个**渲染器**返回给**app 实例**，然后对**实例**的`mount`方法进行扩展，接下来进入实例的**渲染阶段**。
...
- 作用
  - 根据不同的**虚拟 dom**类型 进行 **组件（包括浏览器 host 组件和自定义组件）初始化与更新**，最终将**虚拟 dom**转成**真实 dom**
- 核心源码
  ...
  由于**初始化**时传入的**新的虚拟 dom（`n2`）**是开发者调用`createApp`的参数，所以被判定是一个**对象类型**，接下来执行`processComponent`方法<br>
  核心源码如下：
