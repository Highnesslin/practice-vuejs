...
我的理解：**虚拟 dom** 是用 **js 对象** 描述 **dom** 的一种**数据结构**。**Vue** 对于 **虚拟 dom** 的命名是 `VNode`，是 **Vue2** 中，**Watcher** 的 **更新粒度** 上升到**组件级别**后，为避免**全量更新**而不得不引入的内容。

## 从源码探究流程

上个模块整理了**如何**更新，这个模块具体看下**怎样**更新

### 初始化流程的最终阶段

Vue 初始化最后一步是调用`$mount`,`$mount`方法经过多次**覆盖**，最终执行`mountComponent`,
`mountComponent`在初始化时赋值如下：<br>
...
创建 **Watcher** 时将传入的第二个参数（`updateComponent`）赋值给`get`并执行一次，<br>
那么真正的渲染函数就`_render`和`_update`中

### `_render` [/src/core/instance/render.js](https://github.com/vuejs/vue/blob/dev/src/core/instance/render.js#L69)

- 作用
  - 取出`options`中的`render`函数，传入参数`createElement`并执行，最终产生**虚拟 dom**
- 核心源码
  ...
  **（上文）初始化阶段** 我们看到有个小细节，在`/src/platforms/web/entry-runtime-with-compiler.js`的覆盖`$mount`函数中，做了一个很重要的操作，将`template`转成`render`函数，编译器会将**模板**转成如下 `render` 函数(转成**别名函数**之前)的形式
  ...
  `vm.$createElement`是经过**标准化处理**的**高阶函数**，<br>`vm.$createElement = (a, b, c, d) => createElement(vm, a, b, c, d, true)`<br>
  ...
  如果是浏览器原始标签，则直接通过`new VNode`创建**虚拟 dom**，如果是组件，则需要获取到组件的**构造函数(Ctor)**，通过`createComponent`获取**虚拟 dom**

  由此可见，最终`_render`的最终目的是产生虚拟 dom

### `_update` [/src/core/instance/lifecycle.js](https://github.com/vuejs/vue/blob/dev/src/core/instance/lifecycle.js#L59)

- 作用
  - 1. 保存**虚拟 dom**，用于下次**更新**时取出来使用保存一份`vnode`，
  - 2. 调用`__patch__`将 **虚拟 dom** 转成 **真实 dom**
- 核心源码
...
可以看到 **patch** 包括两种情况，**首次渲染** 和 **组件更新**<br>

- **首次渲染**时传入的旧节点是**真实 dom**
- **组件更新**时传入的新旧节点都是**虚拟 dom**

### `__patch__`: **首次渲染** 和 **组件更新** [/src/core/vdom/patch.js](https://github.com/vuejs/vue/blob/dev/src/core/vdom/patch.js#L700)

- 作用
  - 1. 通过工厂函数`createPatchFunction`返回一个真正的`patch`函数
  - 2. 执行**首次渲染**或**组件更新**，区分方式是：**旧节点**是否是**真实 dom**
    - 2.1. **首次渲染**时，首先调用`createElm`创建一整棵树，添加到旧节点到兄弟节点位置，然后删除旧节点
    - 2.2. **组件更新**时调用`patchVnode`
- 核心源码
...
- 作用
  ...
  1. 按照 **web 场景** 中常见的**列表变化**进行 `diff`，<br>
     常见的场景有
...
1. `sameVnode`在判断时，即使**dom**没有变化也会判定为发生变化，最明显的问题时所有 dom 都要重新更新一次
...
编写 `template` 时的**空格**或**换行符**在`template`转**render 函数**时会被解析成`tag`为`undefined`的**VNode**
