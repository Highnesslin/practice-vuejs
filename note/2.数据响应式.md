...
- 作用
  ...
  1. 为 **对象**/**数组** 创建 **Observer**，并且挂载到对象的`__ob__`属性上，
  2. 因为**数组**无法被`Object.defineProperty`拦截，所以只对数组里的元素执行`observe`
  3. 创建 **dep**，用于**数组响应式**和`set`时使用
...
- 作用：
  1. 通过`Object.defineProperty`为**对象**的**key**进行拦截，**getter** 负责添加依赖，**setter** 主要负责两件事，
     - 1. **通知 dep 更新 watcher**
     - 2. **观察新设置的值(新设置的值可能是个对象)**
...
- 核心源码
  ...
  或许这里你会有个小疑问，执行`setter`时为什么只改形参`val`呢？<br/>
...
- 作用
  - 数组有 7 个改变元素的方法，对这 7 个方法添加额外的功能
    - 1.  **有可能新添加了元素进来，于是观察一下新来的元素**
    - 2.  **取出数组身上的`__ob__`，让他的`dep`通知`watcher`更新视图**
- 核心源码
...
- 作用：
  1. **Dep** 负责管理一组 **Watcher**，包括 **watcher** 实例的增删及通知更新
  2. **Dep** 和 **Watcher** 相互引用，可以通知 **Watcher** 互相添加，也可以通知 **Watcher** 互相删除
- 核心源码
...
1. 初始化时调用`this._init`方法用于**Vue**的初始化工作，其中`initState`方法用于初始化**响应式数据**
2. 首先将`_data`代理到实例上，方便开发者通过`this`调用，然后对`_data`进行响应式处理
3. 每个对象拥有一个**dep**，用于**数组**和`Vue.set`使用，每个**key**也拥有一个**dep**，`getter`时收集依赖（watcher），`setter`时通知依赖（watcher）更新
4. 对于数组采用方法覆盖的方法，7 个方法在执行时追加一个额外的操作，观察新增的元素，然后让数组的**观察者**通知**watcher**进行更新
...
4. 综合**数据响应式原理**，感觉最复杂的部分在于处理 **数组** 和 **新增 key** 的情况，处理这两种情况的大量逻辑都在**Watcher**中，导致**Watcher**看起来很麻烦，这也是后来**Vue3**着重优化的一部分。后续专门整理下**Vue3**的变化以作对比 🚩
