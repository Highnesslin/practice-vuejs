...
**组件 Watcher**在创建时会执行一次组件的`render`函数，从而间接触发相关`key`的`getter`方法，将**Watcher**收集到`key`的`dep`中，
当我们更改`key`的值时会触发`key`的`setter`方法，通过`key`的`dep`通知**Watcher**进行更新。

## 从源码探究流程

初始化执行`_init`时有一个函数叫做`initState`，这便是**数据响应式**的入口
...
- 核心源码
  ...
  即使只选取了 **Watcher** 的核心源码，但内容依然很多，主要包括**重新收集依赖**的过程，抛开这些不谈的话，其实**Watcher**只有初始化时一次更新，以及后续更新使用的`update`方法。

  需要注意的`get`方法，最终的渲染和更新都会走到这里，并且里面有一个`popTarget`方法，这是`Vue.set`的关键，

  `Vue.set`的**核心源码**只有两行，大部分逻辑都在**Watcher**中
  ...
  `notify`方法调用`get`会执行到`popTarget`，将当前**Watcher**赋值给`Dep.target`，然后触发组件的`render`，间接触发`key`的`getter`方法，完成收集依赖。

## 流程梳理

1. **Vue**初始化时调用`this._init`，其中`initState`方法用于初始化**响应式数据**
2. 首先将`_data`代理到实例上，方便开发者通过`this`调用，然后对`_data`进行响应式处理，每个被观察的`value`都多了一个`__ob__`属性，其实是当前对象的**观察者实例**
3. 每个**对象**拥有一个**dep**，用于**数组**和`Vue.set`使用，每个**key**也拥有一个**dep**，`getter`时收集依赖（watcher），`setter`时通知依赖（watcher）更新
...
3. 为什么需要`Vue.set`，其使用场景需要注意什么？<br>

   - 存在的意义：因为`Object.defineProperty`无法对对象所有的`key`进行监听，当动态增加`key`时需要手动设置成响应式。
   - 注意：添加 key 的这个对象必须是响应式，因为`Vue.set`的原理是先设置响应式，然后取出对象身上的**dep**触发更新完成收集依赖，如果对象不是响应式数据就不存在**dep**，因此无法完成依赖收集
