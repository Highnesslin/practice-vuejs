...
- 核心源码
  ...
  需要注意的`get`方法，最终的渲染和更新都会走到这里，并且里面有一个`popTarget`方法，这是`Vue.set`的关键，该方法会将当前**Watcher**赋值给`Dep.target`，更新时为新注入的`key`收集依赖。
...
### `set` 和 `delete` [/src/core/observer/index.js](https://github.com/vuejs/vue/blob/dev/src/core/observer/index.js#L201)
...
- 核心源码
  ...
  这里核心代码只有两行`defineReactive`和`ob.dep.notify()`，重点在后者，`notify`调用`popTarget`时会将当前**Watcher**赋值给`Dep.target`，有了`Dep.target`，接下来在`render`中调用新加入 key 的 getter 方法时就完成了收集依赖
