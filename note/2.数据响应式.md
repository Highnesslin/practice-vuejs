...
- 核心源码
  ...
  - 1.  `defineReactive`方法将新加入的元素会变成响应式，为其次内部创建了一个**dep**
  - 2.  取出对象身上的 **dep**，调用 `notify` 方法，重点就在这里，调用了`pushTarget`<br>
        `pushTarget`源码如下
        ```javascript
        ...
        ```
        有了`Dep.target`，接下来在`render`中调用新加入 key 的 getter 方法时就完成了收集依赖
